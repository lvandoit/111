...
def train_human(train_loader, misc, stat_2d, stat_3d,
    ...
    for i, train_data in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        model_outputs, model_scale, middel_out = model(model_inputs)
        ...
        if standardize_output_data:
            ...
            #中间姿态
            middel_out=outputs_mean + middel_out * outputs_std
            assert use_rel_loss == False, "Cannot use 3d data for relative_loss!"
        ...
        if not use_rel_loss:
            ...
            #中间姿态
            middel_out = middel_out + tars_root
            ####################################################################
            # SUPERVISED LOSS, requires the data to be normalized
            sup_loss = mse_loss(outputs, targets)
            losses_sup.update(sup_loss.item(), batch_size)
            #中间姿态
            sup_loss1 = mse_loss(middel_out, targets)
            losses_mid.update(sup_loss1.item(), batch_size)
            if np.isnan(sup_loss.mean().item()):
            ...
            if reprojection == 'scaled_orthographic':
                ...
                rep_loss1 = loss_weights['reproj']*\
                           rel_losses.reproj_loss_scaled_orthographic(middel_out, inputs, focal_length_over_dist)
                ...
                #中间
                rep_loss1=loss_weights['reproj']*\
                           rel_losses.reproj_loss_estimated_weak_perspective(middel_out, inputs, inverse_dist, f_batch)
            ...
            cam_loss1 =rel_losses.camera_coord_3d_loss(misc, model_outputs, middel_out,
                                                        limb_lens_3d, loss_weights=loss_weights)
            ...
            rel_loss_total = rel_loss + rep_loss +rel_loss1+rep_loss1+cam_loss#+cam_loss1
