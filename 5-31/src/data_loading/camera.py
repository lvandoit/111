...
def project_point_radial(P, R, T, f, c, k, p):
    ...
    N = P.shape[0]
    X = R.dot(P.T - T)  # rotate and translate - X is in camera coords
    D = X[2, ]          # depth in the camera frame
...
def load_camera_params(hf, path, dtype):
    """
    ...
    return R.astype(dtype), T.astype(dtype), f.astype(dtype), c.astype(dtype), k.astype(dtype), p.astype(dtype), name


def load_cameras(bpath='cameras.h5', subjects=None, dtype=np.float32):
    """
    :param bpath: *.h5
    :param subjects:
    :return: (dict)
