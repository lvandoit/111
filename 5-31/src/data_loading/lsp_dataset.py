...
def _makeGaussian(height, width, sigma = 3, center=None):
        x = torch.arange(0, width ).float().cuda()
        y = torch.arange(0, height).unsqueeze(1).float().cuda()
        if center is None:
                x0 =  width // 2
                y0 = height // 2
        else:
...
def _generate_hm( height, width ,joints,  weights=torch.ones(14)):
        num_joints = joints.shape[0]
        ground_hm = torch.zeros((num_joints,height, width), dtype = torch.float32).cuda()
        for i in range(num_joints):
                if   weights[i] == 1:
                        #s = int(np.sqrt(maxlenght) * maxlenght * 10 / 4096) +2 
                        ground_hm[i,:,:] = _makeGaussian(height, width, sigma= 3, center= (joints[i,0], joints[i,1]))
                else:
                        ground_hm[i,:,:] = torch.zeros((height,width))
